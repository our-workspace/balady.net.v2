...
const FastLoading = React.memo(() => (
  <div className="flex justify-center items-center h-screen bg-white">
    <div className="relative flex flex-col items-center justify-center">
      <span className="absolute inline-flex h-12 w-12 rounded-full bg-grey opacity-75 animate-ping" />
...
// عرض بيانات الشهادة
const CertificateDisplay = React.memo(
  ({ certificateData }: { certificateData: Certificate }) => {
    const certificateFields = useMemo(
      () => [
        ...
        {
          label: "رقم المنشأة",
          value: certificateData.facility_number || "",
        },
      ],
      [certificateData],
    )
...
// المكوّن الرئيسي للمسار /verify/[id]
export default function VerifyCertificate() {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "حدث خطأ أثناء جلب بيانات الشهادة"
  ...
  return (
    ...
    >
      ...
      {/* زر الوصول (تصميم muneer) */}
      <style
        id="muneer-loader-css"
        dangerouslySetInnerHTML={{
          __html: `
#muneer-trigger-button{ position: relative; }
.muneer-trigger-button-box{position: relative;}
.muneer-trigger-button-box.loading:before{
  z-index: 0;
  animation: muneer-rotate360 .5s linear infinite;
  content: "";
  width: 100%;
  height: 100%;
  background: conic-gradient(#57cefb 0% 10%, transparent 10% 100%);
  position: absolute;
  top: 0;
  left: 0;
  border-radius: 50%
}
@keyframes muneer-rotate360 {
  from {transform: rotate(0deg) scale(1.1);}
  to {transform: rotate(360deg) scale(1.1);}
}
.muneer-trigger-button-box{position:fixed;z-index:99998;margin:10px;}
.muneer-trigger-button-box.bottom-right{bottom:0;right:0}
...
.muneer-trigger-button-box button:hover .muneer-trigger-button-icon svg{
  fill:rgba(0, 108, 242, 1);
}
.muneer-trigger-button-box button span{display:flex;transition:all .15s linear}
html {
...
div#muneer-action-image-alt { display: none; }
        `,
        }}
      />
      <div className="muneer-trigger-button-box bottom-right entrance-fade hover-none">
        <button
          id="muneer-trigger-button"
          className="icon-position-before"
          data-muneer-trigger=""
      ...
      {/* Footer محسن */}
      <footer className="footer footer-inner bg-gray-50 py-3">
        <div className="container-fluid">
          <div className="footer-cont flex flex-col md:flex-row justify-between items-center">
            <div className="side-one mb-2 md:mb-0 flex flex-col items-center gap-1">
              <a
              ...
              >
                {/* يمكن استبدال SVG بمكون منفصل لتحسين الأداء */}
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                ...
                >
                  ...
                  <switch>
                    <foreignObject
                      ...
                      x="0"
                      y="0"
                      requiredExtensions="https://balady-gov.net/ns.adobe.com/AdobeIllustrator/10.0/index.html"
                    />
                    <g>
